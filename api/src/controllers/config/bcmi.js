...
exports.CreateBCMIConfig = async function (data, author) {
  ...
  configData.write = [utils.ApplicationRoles.ADMIN_BCMI, utils.ApplicationRoles.ADMIN];
  ...
  switch (data.configType) {
    case ConfigConsts.CONFIG_TYPES.enforcementActionText:
      // Data fields: text
      if (data.text) {
        configData.data['text'] = data.text;
      } else {
        throw 'You must include text in data for configType enforcementActionText.';
      }
      break;
...
exports.EditBCMIConfig = async function (_id, data, author) {
  ...
  return await ConfigData.findOneAndUpdate({ _id: _id }, configData);
};

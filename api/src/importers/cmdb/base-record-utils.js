...
class BaseRecordUtils {
   ...
   *
   * @param {*} nrptiRecord
   * @returns {object} existing NRPTI master record, or null if none found or _sourceRefAgriCmdbId Id is null
   * @memberof BaseRecordUtils
   */
  async findExistingRecord(nrptiRecord) {
    if (!nrptiRecord._sourceRefAgriCmdbId) {
      return null;
    }
    ...
    return await masterRecordModel
      .findOne({
        _schemaName: this.recordType._schemaName,
        _sourceRefAgriCmdbId: nrptiRecord._sourceRefAgriCmdbId
      })

...
> _Note: you may need to adjust Minio env variables, or skip the loadMemDocs migration_ 4. Run the app
...
This project is using [jest](http://jestjs.io/) as a testing framework. You can run tests with `yarn test` or `jest`. Running either command with the `--watch` flag will re-run the tests every time a file is changed.
...
test('GET /api/document/:id  returns 200', done => {
  request(app).get('/api/document/AAABBB').expect(200).then(done);
});
```
...
beforeEach(() => {
  bcgw.get(searchPath + urlEncodedDispositionId).reply(200, crownlandsResponse);
});
...
- `era-csv`
  - Imported as a CSV from the Natural Resource Office: The Enforcement Action, Administrative Review and Appeal Tracking System (NRO-ERA)
- `ams-csv`
...
- 2.  BCLaws-API-URL: each act is associated with a unique code and endpoint. There are at least two different endpoints that return XML. Use the one that ends in “\_01/xml”. ( Example endpoint : https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/08036_01/xml )

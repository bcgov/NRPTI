...
exports.up = async function(db) {
  ...
  try {
    ...
    const currentLNGRecords = await nrptiCollection.find({sourceSystemRef: { $in: ['lng-csv']}, _epicProjectId: new ObjectId("588511d0aaecd9001b826192")}).toArray()
    ...
    for (let i = 0; i < currentCGLRecords.length; i++) {
      ...
      await nrptiCollection.update({ _id: _id },
                                   { $set: { _epicProjectId: new ObjectId("588511d0aaecd9001b826192"), projectName: "LNG Canada" }
      })
    }
    ...
    for (let z = 0; z < currentActivityRecords.length; z++) {
      ...
      const swappedID = currentActivityRecords[z].url.includes('/project/1') ? new ObjectId("588511d0aaecd9001b826192") : new ObjectId("588511c4aaecd9001b825604")
      await nrptiCollection.update({ _id: _id },

...
import { OrderNRCED } from '../../../../../../common/src/app/models/nrced/order-nrced';
...
@Component({
  selector: 'app-order-nrced-detail',
  templateUrl: './order-nrced-detail.component.html',
  styleUrls: ['./order-nrced-detail.component.scss']
})
export class OrderNRCEDDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new OrderNRCED(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.ORDER.NRCED;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new OrderNRCED(response);
        this.isPublished = this.isRecordPublished();

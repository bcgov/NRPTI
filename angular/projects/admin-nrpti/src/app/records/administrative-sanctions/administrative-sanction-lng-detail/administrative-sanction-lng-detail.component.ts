...
import { AdministrativeSanctionLNG } from '../../../../../../common/src/app/models/lng/administrative-sanction-lng';
...
export class AdministrativeSanctionLNGDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new AdministrativeSanctionLNG(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.ADMINISTRATIVE_SANCTION.LNG;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new AdministrativeSanctionLNG(response);
        this.isPublished = this.isRecordPublished();

...
import { WarningLNG } from '../../../../../../common/src/app/models/lng/warning-lng';
...
@Component({
  selector: 'app-warning-lng-detail',
  templateUrl: './warning-lng-detail.component.html',
  styleUrls: ['./warning-lng-detail.component.scss']
})
export class WarningLNGDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new WarningLNG(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.WARNING.LNG;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new WarningLNG(response);
        this.isPublished = this.isRecordPublished();

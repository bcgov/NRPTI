...
import { SelfReportLNG } from '../../../../../../common/src/app/models/lng/self-report-lng';
...
@Component({
  selector: 'app-self-report-lng-detail',
  templateUrl: './self-report-lng-detail.component.html',
  styleUrls: ['./self-report-lng-detail.component.scss']
})
export class SelfReportLNGDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new SelfReportLNG(records[0]);
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new SelfReportLNG(response);
        this.isPublished = this.isRecordPublished();

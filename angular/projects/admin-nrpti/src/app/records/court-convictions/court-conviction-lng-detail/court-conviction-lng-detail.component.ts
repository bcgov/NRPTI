...
import { CourtConvictionLNG } from '../../../../../../common/src/app/models/lng/court-conviction-lng';
...
@Component({
  selector: 'app-court-conviction-lng-detail',
  templateUrl: './court-conviction-lng-detail.component.html',
  styleUrls: ['./court-conviction-lng-detail.component.scss']
})
export class CourtConvictionLNGDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new CourtConvictionLNG(records[0]);
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new CourtConvictionLNG(response);
        this.isPublished = this.isRecordPublished();

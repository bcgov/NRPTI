...
import { CourtConvictionBCMI } from '../../../../../../common/src/app/models/bcmi/court-conviction-bcmi';
...
export class CourtConvictionBCMIDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new CourtConvictionBCMI(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.COURT_CONVICTION.BCMI;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new CourtConvictionBCMI(response);
        this.isPublished = this.isRecordPublished();

...
import { RestorativeJusticeNRCED } from '../../../../../../common/src/app/models/nrced/restorative-justice-nrced';
...
@Component({
  selector: 'app-restorative-justice-nrced-detail',
  templateUrl: './restorative-justice-nrced-detail.component.html',
  styleUrls: ['./restorative-justice-nrced-detail.component.scss']
})
export class RestorativeJusticeNRCEDDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new RestorativeJusticeNRCED(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.RESTORATIVE_JUSTICE.NRCED;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new RestorativeJusticeNRCED(response);
        this.isPublished = this.isRecordPublished();

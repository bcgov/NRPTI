...
export class TicketAddEditComponent implements OnInit, OnDestroy {
  ...
  async submit() {
    ...
    if (this.myForm.controls.projectName.dirty) {
      ticket['projectName'] = this.myForm.controls.projectName.value;
    ...
    if (!this.isEditing) {
      const res = await this.factoryService.writeRecord(ticket, 'tickets', true);
      this.recordUtils.parseResForErrors(res);
      let _id = null;
      if (Array.isArray(res[0][0].object)) {
        _id = res[0][0].object.find(r => r._schemaName === 'Ticket')._id;
      } else {
      ...
      const res = await this.factoryService.writeRecord(ticket, 'tickets', false);
      this.recordUtils.parseResForErrors(res);
      const docResponse = await this.recordUtils.handleDocumentChanges(
        this.links,
        this.documents,

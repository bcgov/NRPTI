...
export class Picklists {
  ...
  public static getAgencyNames(factoryService: FactoryServiceNRPTI) {
    const agencyDataService = new AgencyDataService(factoryService);
    return agencyDataService.getAgencyNames();
  }

  public static getAgencyCode(factoryService: FactoryServiceNRPTI, agencyName) {
    const agencyDataService = new AgencyDataService(factoryService);
    return agencyDataService.getAgencyCode(agencyName);
  }

  public static getAgencyCodes(factoryService: FactoryServiceNRPTI) {
    const agencyDataService = new AgencyDataService(factoryService);
    return agencyDataService.getAgencyCodes();
  }

  public static getAllActs(factoryService: any) {
    let actDataService;
    switch (true) {
  ...
  public static readonly legislationDescriptions = {
    AdministrativePenalty: {
      ...
      'Environmental Assessment Act': {
        '60': {
          description:
            'Penalty for contravention of this Act or regulations, failure to comply with an order or environmental certificate issued under this Act, or making false or misleading statement in a record filed under this Act'
        }
      }
    },
    AdministrativeSanction: {

  ...
  try {
    ...
    for (let i = 0; i < publishedMines.length; i++) {
      try {
        ...
        promises.push(updateMineLinks(mineData, nrpti));
      } catch(err) {
    ...
    // we're done
    console.log('BCMI external links migration complete.');
    ...
    console.error('Error on BCMI external links dataload: ' + err);
  }
  return null;
};
...
async function updateMineLinks(mineData, nrpti) {
  let nrptiMines = [];
  ...
  if (nrptiMines.length === 1) {
    ...
    console.log("Updating links for:", mineData.name);

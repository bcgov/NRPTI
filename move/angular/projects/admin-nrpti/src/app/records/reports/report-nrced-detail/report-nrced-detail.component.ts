...
import { ReportNRCED } from '../../../../../../common/src/app/models/nrced/report-nrced';
...
@Component({
  standalone: false,
  selector: 'app-report-nrced-detail',
  templateUrl: './report-nrced-detail.component.html',
  styleUrls: ['./report-nrced-detail.component.scss']
})
export class ReportNRCEDDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        if (!res || !res.records) {
          alert("Uh-oh, couldn't load Report");
        ...
        this.data = records && records[0] && new ReportNRCED(records[0]);
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new ReportNRCED(response);
        this.isPublished = this.isRecordPublished();

...
@Component({
  standalone: false,
  selector: 'app-administrative-sanction-nrced-detail',
  templateUrl: './administrative-sanction-nrced-detail.component.html',
  styleUrls: ['./administrative-sanction-nrced-detail.component.scss']
})
export class AdministrativeSanctionNRCEDDetailComponent extends RecordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.data) {
      this.route.data.pipe(takeUntil(this.ngUnsubscribe)).subscribe((res: any) => {
        ...
        this.data = records && records[0] && new AdministrativeSanctionNRCED(records[0]);
  ...
  canPublish(): boolean {
    const requiredRoles = Constants.FlavourEditRequiredRoles.ADMINISTRATIVE_SANCTION.NRCED;
  ...
  publish(): void {
    this.factoryService
      ...
      .subscribe(response => {
        ...
        this.data = new AdministrativeSanctionNRCED(response);
        this.isPublished = this.isRecordPublished();

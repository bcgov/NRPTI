...
export class InspectionAddEditComponent implements OnInit, OnDestroy {
  ...
  async submit() {
    ...
    if (!this.isEditing) {
      const res = await this.factoryService.writeRecord(inspection, 'inspections', true);
      ...
      const res = await this.factoryService.writeRecord(inspection, 'inspections', false);
      this.recordUtils.parseResForErrors(res);
      const docResponse = await this.recordUtils.handleDocumentChanges(
        this.links,
        this.documents,
